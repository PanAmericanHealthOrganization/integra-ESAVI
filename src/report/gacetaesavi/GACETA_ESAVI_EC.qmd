...
#pacman::p_load(dplyr, magrittr, RJDBC, rmarkdown, rJava, tidyverse, leaflet, flextable, lubridate, colorspace, sqldf, scales, stringi, janitor, kableExtra, emojifont, cowplot)
...
con <- dbConnect(drv, url="jdbc:postgresql://10.64.102.47:5432/dhi_esavi", user="usr_dhis2esavi", password="usr_dhis2esavi")
...
tdosis <- colSums(Dosis_vac[2:5], na.rm = T)
...
Tab2 |> 
  kbl(booktabs = T, col.names = c("Vacuna","Dosis 1","Dosis 2", "Refuerzo 1", "Refuerzo 2", "Total")) |> 
...
En la [Figura @fig-vacunas] se muestran los porcentajes del tipo de vacunas inoculadas hasta el corte del presente informe.
...
casos <- "select TC.\"DESCRIPCIONHOMOLOGADA\" provincia, count(*) esavis_graves_por_provincia
...
tb_casos <- dbGetQuery(con, casos)

casos_prov <- left_join(provincias_sf, tb_casos, by = c("DPA_DESPRO" = "provincia")) |>
  select(3:5)
```
...
```{r}
casos_prov$esavis_graves_por_provincia <- as.numeric(casos_prov$esavis_graves_por_provincia)

Ec_continental <- ggplot(data = casos_prov) + geom_sf(aes(fill = esavis_graves_por_provincia), lwd = 0.1) + scale_fill_continuous_sequential(palette = "Blues", rev = TRUE) + 
  coord_sf(xlim = c(-81, -75), ylim = c(-5, 2), expand = FALSE) + labs(x = "", y = "") + guides(fill = guide_legend(title = "ESAVIs")) +
  theme_classic()

Ec_insular <- ggplot(data = casos_prov) + geom_sf(aes(fill = esavis_graves_por_provincia), lwd = 0.1) + scale_fill_continuous_sequential(palette = "Blues", rev = TRUE) + 
  coord_sf(xlim = c(-92, -90), ylim = c(-2, 1), expand = FALSE) +
  theme(legend.position = "none", axis.title.x = element_blank(), axis.title.y = element_blank(), panel.background = element_rect(fill = "azure"), panel.border = element_rect(fill = NA))
```

```{r}
#| label: fig-maprov
#| fig-cap: Número total de casos ESAVI graves por provincia

plot_grid(Ec_continental, Ec_insular, nrow = 1, rel_widths = c(2.5, 1))
```
...
```{r}
#| label: tbl-esavisexanio
#| tbl-cap: ESAVIs por sexo

tb_esavi_sex$count <- as.numeric(tb_esavi_sex$count)

tb_esavi_sex_anio <- tb_esavi_sex |> 
  group_by(`año`, DESCRIPCIONHOMOLOGADA) |> 
  summarise(casos = sum(count))

kbl(tb_esavi_sex_anio, booktabs = T, col.names = c("Año","Sexo","Número de casos")) |> 
  row_spec(0, bold = TRUE) |> 
  kable_styling(latex_options = c("HOLD_position"))
```

En el periodo desde el 21 de enero del 2021 al 23 de marzo de 2023. Los ESAVI GRAVES según sexo, al masculino le corresponde el **49% (48 casos)** (@tbl-esavisexanio) con una tasa de reporte de 0,14 casos por 100 000 dosis inoculadas; al femenino le corresponde el **51% (50 casos)** con una tasa de reporte de 0,14 casos por 100 000 dosis inoculadas.
...
Tab2_modif <- Dosis_vac |> 
  mutate(suma_vac = rowSums(across(.cols = 2:5), na.rm = TRUE), 
         tasa_vac = round(suma_vac/100000), 3)
  
  
#data.frame(matrix(Datos3,2,1,byrow=T))
#colnames(Tab3) <- "Tasa"
...
kbl(Tab2_modif[ , c(1,7)], booktabs = TRUE, col.names = c("Vacuna", "Tasa ESAVI graves")) %>%
  row_spec(0, bold = TRUE) |> 
  footnote(general = c("\\\\footnotesize{Fuente: Ministerio de Salud Pública. SIVE ALERTA}","\\\\footnotesize{Elaborado por: Estrategia Nacional de Inmunizaciones}"), general_title = "", escape = F) %>% 
...
```{r}
esavi_grupo_etario <- "select ge.\"INICIO\", coalesce (ge.\"DESCRIPCION\", 'No registra la edad'), count(*) cantidad_registro
...
tb_esavi_grup_etario <- dbGetQuery(con, esavi_grupo_etario)
```

```{r}
#| label: tbl-esav-grup-etar
#| tbl-cap: Casos ESAVI por grupo etario
tb_esavi_grup_etario |> 
  select(2,3) |> 
  kbl(booktabs = T, col.names = c("Grupo etario", "Casos")) |> 
  row_spec(0, bold = TRUE) |> 
  kable_styling(latex_options = c("HOLD_position"))
```
...
```{r}
esavi_grav_caus <- "select tc.\"DESCRIPCIONHOMOLOGADA\", count(*) causalidad from dhi_esavi.\"TR_PACIENTE\" P inner join  
		dhi_esavi.\"TR_NOTIFICACION\" tn on p.\"PACIENTE_ID\" = tn.\"PACIENTE_ID\"  inner join  
  	 	dhi_esavi.\"TR_GRAVEDADESAVI\" tg on tg.\"NOTIFICACION_ID\" =  tn.\"NOTIFICACION_ID\" inner join 
...
tb_esavi_grav_caus <- dbGetQuery(con, esavi_grav_caus)
```


```{r}
#| label: tbl-esavigravcaus
#| tbl-cap:  ESAVI GRAVE por análisis de causalidad
kbl(tb_esavi_grav_caus, booktabs = TRUE, col.names = c("Causa", "Casos")) |> 
  row_spec(0, bold = TRUE) |> 
...
```{r}
sint_esavi_no_grav <- "select llt.\"LLT_NAME\",  COUNT(*) síntomas_mayormente_reportados
from dhi_esavi.\"TR_PACIENTE\" P inner join  
		dhi_esavi.\"TR_NOTIFICACION\" tn on p.\"PACIENTE_ID\" = tn.\"PACIENTE_ID\"  inner join 
...
tb_sint_esavi_no_grav <- dbGetQuery(con, sint_esavi_no_grav)
```

```{r}
#| label: tbl-esavingsint
#| tbl-cap: ESAVI no grave por signos y síntomas mayormente reportados

kbl(tb_sint_esavi_no_grav, booktabs = TRUE, col.names = c("Signo/Síntoma", "Casos")) %>%
  row_spec(0, bold = TRUE) |> 
  footnote(general = c("\\\\footnotesize{Fuente: Agencia Nacional de Regulación, Control y Vigilancia Sanitaria (ARCSA)}","\\\\footnotesize{Elaborado por: Estrategia Nacional de Inmunizaciones}"), general_title = "",escape = F) %>% 
kable_styling(latex_options = "HOLD_position")
...
```{r}
#| label: fig-esavingsint
#| fig-cap: ESAVI no grave por signos y síntomas
tb_sint_esavi_no_grav$síntomas_mayormente_reportados <- as.numeric(tb_sint_esavi_no_grav$síntomas_mayormente_reportados)

tb_sint_esavi_no_grav$pct = tb_sint_esavi_no_grav$síntomas_mayormente_reportados / sum(tb_sint_esavi_no_grav$síntomas_mayormente_reportados) * 100
tb_sint_esavi_no_grav$label = sprintf("%s (%s%%)", tb_sint_esavi_no_grav$síntomas_mayormente_reportados, round(tb_sint_esavi_no_grav$pct, 1))

ggplot(tb_sint_esavi_no_grav, aes(x = reorder(LLT_NAME,-síntomas_mayormente_reportados), síntomas_mayormente_reportados, label = label)) +
  geom_col() +
  geom_text(size = 2, position = position_dodge(width = 1), vjust = -0.25) + labs(x = "", y = "Frecuencia") + theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust = 0.5))
