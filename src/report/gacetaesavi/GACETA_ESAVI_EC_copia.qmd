...
pacman::p_load(tidyverse,sf,leaflet,flextable,lubridate,colorspace,RPostgreSQL, sqldf, scales)
...
drv <- JDBC("org.postgresql.Driver","C:/Program Files/RStudio/postgresql-42.5.1.jar")

con <- dbConnect(drv, url="jdbc:postgresql://10.64.102.29:5432/dhis2", user="usr_dhis2esavi", password="aaaDHIS2")


consulta <- dbGetQuery(con, "select count(*) totalPersonas from dhi_esavi.tr_paciente")
...
Consulta4<-  dbGetQuery(con," select nombrevacuna, numerodosis, sum(cantidad) 
...
```{r}
Datos1 <- c(4288129, 4018529, 2326667, 341940, 10975265, 8616193 , 8045243, 444193) 
Tab2<- data.frame(matrix(Datos1,2,4,byrow=T))
colnames(Tab2)<- c("dosis_1","dosis_2","refuerzo_1","refuerzo_2")
Tab2$Vacunas<- c("Pfizer","Sinovac")
Tab2<- Tab2 %>% select(5,1:4)
totalvac<- sum(Datos1)
resp<- colSums(Tab2[,2:5])
res1<- form_mil(resp[1])
res2<- form_mil(resp[2])
res3<- form_mil(resp[3])
res4<-form_mil(resp[4])
Tab2_2<-Tab2 %>% mutate(across(.cols = c(dosis_1,dosis_2,refuerzo_1,refuerzo_2),.fns = form_mil))

Pfizer<- rowSums(Tab2[1,2:5])
Sinovac <- rowSums(Tab2[2,2:5])
...
Entre el 21 enero de 2021 al 25 de julio del 2022, se han inoculado un total de `r form_mil(totalvac)` dosis, de las cuales,primeras dosis corresponden a `r res1`, segundas dosis corresponde a `r res2`, primer refuerzo corresponde a `r res3` dosis, segundo refuerzo corresponde a `r res4`.

```{r}
#| label: tbl-dosis_vac_reg
#| tbl-cap:  Número de dosis inoculadas 

kable(Tab2_2, format= 'latex', booktabs = TRUE) %>% footnote(general = c("\\\\footnotesize{Fuente: Vacunómetro }","\\\\footnotesize{Elaborado por: DNVE-DNI}"),general_title = "",escape = F) %>% 
kable_styling(latex_options = "HOLD_position")

```

Por el tipo de biológico, con corte al `r format(today(), "%d-%b-%Y")`, el `r sinovac_p`% de dosis administradas corresponde a la vacuna `r Tab2[2,1]`, con un total `r form_mil(Sinovac)` de dosis inoculadas, el `r Pfizer_p`% de dosis corresponde a la vacuna `r Tab2[1,1]` con un total de `r form_mil(Pfizer)` de dosis inoculadas, xxxx
