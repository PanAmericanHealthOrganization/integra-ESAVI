...
export class DimConsistenciaService {
    ...
    for (const evalItem of evaluacion) {
      ...
      for (const columna of evalItem.columnas) {
        ...
        resultados.push({
          ...
          descripcionRegla: `Descripción fechas futuras: Determinadas variables de tipo fecha no pueden ser posteriores a la fecha en que se realiza el análisis de calidad.`,
          ...totales,
        });
      }
    }
  ...
  private async _integridadFechaNotificacion(day: Date): Promise<CalidadDatosResultadoDto> {
    ...
    return {
      ...
      condicion: `1.  TR_NOTIFICACION.FECHA_NOTIFICACION ≥ TR_PACIENTE.FECHA_NACIMIENTO
                  2. TR_NOTIFICACION.FECHA_NOTIFICACION ≥  TR_DATO_VACUNACION.FECHA_VACUNACION
                  3. TR_NOTIFICACION.FECHA_NOTIFICACION ≥  TR_DATOS_ESAVI.FECHA_ESAVI
                `,
      descripcionRegla:
  ...
  private async _integridadGravedadEsavi(date: Date): Promise<CalidadDatosResultadoDto> {
    ...
    const query = `
       select
      count(*) filter (where tde."TIPO_GRAVEDAD" ='1') as "totalRegistros",
      count(*) filter (
        where
        (tde."TIPO_GRAVEDAD" = '1' and 
        (
          tde."MUERTE" = true OR
